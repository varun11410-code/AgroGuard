...
import { Users, Scan, Activity, LayoutDashboard, FileText, ShieldCheck, X } from "lucide-react";
...
export function AdminSidebar({ isOpen = false, onClose }: AdminSidebarProps) {
  ...
  return (
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 w-[260px] bg-[#050d07]/95 border-r border-white/5 backdrop-blur-xl z-50 flex flex-col transition-transform duration-300 md:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="p-6 relative">
          ...
          {isOpen && (
            <button 
              onClick={onClose} 
              className="md:hidden absolute top-8 right-6 text-white/50 hover:text-white transition-colors"
            >
              <X size={20} />
            </button>
          )}
            ...
            {links.map((link) => {
              ...
              return (
                <Link 
                  ...
                  href={link.href} 
                  onClick={onClose}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer transition-colors ${isActive ? 'bg-green-500/10 text-green-400 font-medium' : 'text-white/50 hover:text-white hover:bg-white/5'}`}
                >
                  <Icon size={18} />
